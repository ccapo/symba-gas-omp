...
!  Created by Hal and Martin September 2006.  Has gas drag and type I migration.
!
...
integer(ik) :: iflgchk,iub,iuj,iud,iue,ium,iur
...
! common block to store size distribution information
! See drag_size_gen and gen_ray_size for their generation
...
! Set rdrag and dragc to zero for the central object, and all massive particles
rdrag(1:nbodm) = 0.0_rk
dragc(1:nbodm) = 0.0_rk

! Read in drag data
if(nbod > (nbodm + 1)) then

  open(unit = 10, file = 'drag_radius.dat', status = 'old')

  do i = nbodm + 1, nbod

    read(10,'(i9,2(1x,1pe22.15))') id, rdrag(i), dragc(i)
...
if(btest(iflgchk,0)) then ! bit 0 is set
  ...
  call io_write_radius(t0,nbod,rdrag,outfile,iur,fopenstat) ! Writes radii to a separate binary file
...
if(btest(iflgchk,1)) then ! bit 1 is set
  ...
  call io_write_radius_r(t0,nbod,rdrag,outfile,iur,fopenstat) ! Writes radii to a separate binary file
...
do while((t <= tstop) .and. (nbod > 1))
  ...
  call symba5_gas_step_pl(i1st,t,nbod,nbodm,mass,j2rp2,j4rp4,xh,yh,zh,vxh,vyh,vzh,dt,lclose,lgas,rpl,isenc,&
  ...
  if(t >= tout) then
    ...
    if(btest(iflgchk,0)) then ! bit 0 is set
      ...
      call io_write_radius(t,nbod,rdrag,outfile,iur,fopenstat) ! Writes radii to a separate binary file
    ...
    if(btest(iflgchk,1)) then ! bit 1 is set
      ...
      call io_write_radius_r(t,nbod,rdrag,outfile,iur,fopenstat) ! Writes radii to a separate binary file
